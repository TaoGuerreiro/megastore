...
module Filterable
  class Filter
    ...
    delegate :operators, :operators_options, :input_less_operators, to: :class
    ...
    def self.operator_needs_input?(operator)
      !operator.to_s.in?(input_less_operators)
    end
    ...
    def set_to_default_operator!
      self.operator = self.class.operators.keys.first
    end
